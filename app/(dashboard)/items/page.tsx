    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-ink-muted">
          Harga otomatis dihitung dari seluruh Purchase Order yang tersimpan
        </p>
      </div>
      ...
      <Card>
        {items.length ? (
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border text-left text-xs text-ink-muted">
                  ...
                  <th className="px-5 py-3 font-medium">Supplier Terakhir</th>
                </tr>
              </thead>
              <tbody>
                {items.map((item) => (
                  <tr key={item.name} className="border-b border-border last:border-0 hover:bg-bg-subtle">
                    ...
                    <td className="px-5 py-3 text-ink-muted">{item.lastSupplier}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div className="p-10 text-center text-sm text-ink-muted">
            Belum ada data barang. Upload Purchase Order untuk mengisi Master Barang otomatis.
          </div>
        )}
      </Card>
    </div>
